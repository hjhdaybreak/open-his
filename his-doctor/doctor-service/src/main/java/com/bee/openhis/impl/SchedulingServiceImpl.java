...
import com.bee.openhis.mapper.SchedulingMapper;
        ...
        if (schedulingFormDto.getData() != null && schedulingFormDto.getData().size() > 0) {
            ...
            if (userId != null) {
                ...
                for (SchedulingFormDto.SchedulingData datum : schedulingFormDto.getData()) {
                    ...
                    int i = 0;
                    for (String schedulingType : datum.getSchedulingType()) {
                        if (StringUtils.isNotBlank(schedulingType)) {
                            scheduling = new Scheduling(userId, deptId, schedulingDays.get(i),
                                    datum.getSubsectionType(), schedulingType, DateUtil.date(), schedulingFormDto.getSimpleUser().getUserName());
                            //保存数据
